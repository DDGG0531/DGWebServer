...
module.exports = function orderComplete(orderID, memberID) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (hasData === false) {
      ...
      db.query('UPDATE product, order_list SET product.quantity = product.quantity - order_list.order_quantity WHERE order_list.product_id = product.id and order_list.order_id = ?;', orderID, function (err, rows) {
        if (err) {
          console.log(err);
          result.status = "訂單完成失敗。"
        ...
        db.query('UPDATE order_list SET is_complete = 1 WHERE order_id = ?', orderID, async function (err, rows) {
          if (err) {
            console.log(err);
            result.status = "訂單完成失敗。"
            result.err = "伺服器錯誤，請稍後在試！"
